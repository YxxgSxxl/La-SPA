<!-- Footer Menu -->
<footer>
    <div class="footer-p1">
        <a class="footer-logo" href="./index.php?action=accueil" style="filter: brightness(0%);">
            <img src="./img/logospa.png" alt="Logo association site de la SPA" loading="lazy">
        </a>
        <small>Sélectionnez les caractéristiques que vous recherchez pour trouver votre prochain animal de
            compagnie</small>
    </div>
    <div class="footer-p2">
        <h4 class="f-pages">Pages</h4>
        <ul>
            <li><a class='f-lien' href='index.php?action=accueil'>Accueil</a></li>
            <li><a class='f-lien' href='index.php?action=apropos'>À propos</a></li>
            <li><a class='f-lien' href='index.php?action=adopter'>Adopter</a></li>
            <li><a class='f-lien' href='index.php?action=nosactions'>Nos actions</a></li>
            <li><a class='f-lien' href='index.php?action=contact'>Contact</a></li>
            <li><a class='f-lien' href='index.php?action=login'>Se connecter</a></li>
        </ul>
        <h4 class='nrs'>Nos réseaux sociaux !</h4>
        <div class="fp2-socials">
            <!-- instagram -->
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="50" viewBox="0 0 32 32">
                <path
                    d="M 11.46875 5 C 7.917969 5 5 7.914063 5 11.46875 L 5 20.53125 C 5 24.082031 7.914063 27 11.46875 27 L 20.53125 27 C 24.082031 27 27 24.085938 27 20.53125 L 27 11.46875 C 27 7.917969 24.085938 5 20.53125 5 Z M 11.46875 7 L 20.53125 7 C 23.003906 7 25 8.996094 25 11.46875 L 25 20.53125 C 25 23.003906 23.003906 25 20.53125 25 L 11.46875 25 C 8.996094 25 7 23.003906 7 20.53125 L 7 11.46875 C 7 8.996094 8.996094 7 11.46875 7 Z M 21.90625 9.1875 C 21.402344 9.1875 21 9.589844 21 10.09375 C 21 10.597656 21.402344 11 21.90625 11 C 22.410156 11 22.8125 10.597656 22.8125 10.09375 C 22.8125 9.589844 22.410156 9.1875 21.90625 9.1875 Z M 16 10 C 12.699219 10 10 12.699219 10 16 C 10 19.300781 12.699219 22 16 22 C 19.300781 22 22 19.300781 22 16 C 22 12.699219 19.300781 10 16 10 Z M 16 12 C 18.222656 12 20 13.777344 20 16 C 20 18.222656 18.222656 20 16 20 C 13.777344 20 12 18.222656 12 16 C 12 13.777344 13.777344 12 16 12 Z">
                </path>
            </svg>
            <!-- Facebook -->
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="50" viewBox="0 0 48 48">
                <path
                    d="M 11.5 6 C 8.4802259 6 6 8.4802259 6 11.5 L 6 36.5 C 6 39.519774 8.4802259 42 11.5 42 L 36.5 42 C 39.519774 42 42 39.519774 42 36.5 L 42 11.5 C 42 8.4802259 39.519774 6 36.5 6 L 11.5 6 z M 11.5 9 L 36.5 9 C 37.898226 9 39 10.101774 39 11.5 L 39 36.5 C 39 37.898226 37.898226 39 36.5 39 L 30 39 L 30 29 L 33.625 29 C 34.129 29 34.555187 28.623047 34.617188 28.123047 L 34.992188 25.123047 C 35.028188 24.839047 34.938047 24.553891 34.748047 24.337891 C 34.559047 24.122891 34.287 24 34 24 L 30 24 L 30 20.5 C 30 19.397 30.897 18.5 32 18.5 L 34 18.5 C 34.552 18.5 35 18.053 35 17.5 L 35 14.125 C 35 13.607 34.604844 13.174906 34.089844 13.128906 C 34.030844 13.123906 32.619984 13 30.833984 13 C 26.426984 13 24 15.616187 24 20.367188 L 24 24 L 20 24 C 19.448 24 19 24.447 19 25 L 19 28 C 19 28.553 19.448 29 20 29 L 24 29 L 24 39 L 11.5 39 C 10.101774 39 9 37.898226 9 36.5 L 9 11.5 C 9 10.101774 10.101774 9 11.5 9 z">
                </path>
            </svg>
        </div>
    </div>
    <div class="footer-p3">
        <h4>Pour plus d'informations</h4>
        <ul>
            <li><a class='f-lien' href='https://www.la-spa.fr/la-spa-en-france/'>Toutes les SPA de France</a></li>
            <li><a class='f-lien' href='index.php?action=conditions#mentionslegales'>Mentions légales</a></li>
            <li><a class='f-lien' href='index.php?action=conditions#conditionsutilisation'>Conditions générales d'utilisation</a></li>
        </ul>
        <div class="f-buttons">
            <a href="index.php?action=adopter" style="text-decoration: none;"><span class="span-footer1">Je veux adopter</span></a>
            <a href="" style="text-decoration: none;"><span class="span-footer2">Soutenir une SPA</span></a>
        </div>
    </div>
</footer>